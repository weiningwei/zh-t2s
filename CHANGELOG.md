...
- **依赖升级 opencc-js 1.4.0 → 1.4.2**：`@require` 升至当前 latest。经全字表（U+4E00–U+9FFF）逐字比对，两版本间仅 4 处差异且均为修正：
  - 繁→简：`坏` 坯→坏、`牴` 牴→抵、`遶` 遶→绕
  - 简→繁：`托` 託→托（`委托→委託`、`托盘→托盤` 等短语级转换不受影响）
  - 回归抽查：`破壞→破坏`、`委託→委托`、`體重→体重`、`乾隆→乾隆` 均正确

### 修复

- **「坏」被误转为「坯」**：页面上本就是简体字的「坏」会被改写——坏死→坯死、破坏→破坯、太坏了→太坯了、损坏→损坯。
  根源在上游字典：OpenCC `TSCharacters.txt` 给「坏」的两个候选是「坏 坯」，opencc-js 1.4.0 打包时误取了非首选的「坯」；1.4.2 已移除该单字条目，故由依赖升级直接修复，脚本层无需规避。
  升级后 `土坏 → 土坯`、`一坏 → 一坯`（读 pēi）等短语级转换依旧正常。
...
- **FAB 右键快速切换转换方向**：右键点击浮动按钮可直接在繁→简与简→繁之间切换（关闭态则按上次方向开启），无需打开面板。高频操作从 2 步缩为 1 步。
- **面板位置随 FAB 动态调整**：面板不再硬编码钉死在 FAB 上方。当 FAB 位于视口上半部分时，面板在 FAB 下方弹出；位于下半部分时在上方弹出，彻底避免拖到顶部后面板飞出屏幕。
- **快捷键捕获模式视觉引导**：进入快捷键配置状态时，对应配置行高亮黄色背景 + 边框，面板内即有清晰的操作提示（「按下任意键…」），无需观察菜单变化。
- **面板关闭退出动画**：关闭时增加 120ms 淡出 + 缩小动画，与弹出动画对称，交互反馈更完整。

### 变更

- **白名单操作不再刷新页面**：`toggleWhitelist` / `clearWhitelist` 改为就地生效，调用 `applyWhitelistChange()` 实时恢复/启停 observer 并更新 UI，不再调用 `location.reload()`。
- **清空白名单增加确认弹窗**：防止误触清空所有白名单配置。
- **面板宽度 208px → 240px**：放宽宽度避免快捷键配置提示等较长文字换行。
- **面板完全扁平化**：移除设置子视图，所有操作（方向切换、开关、白名单、重置、隐藏）直接铺在主视图。快捷键配置入口一并移除（快捷键在 GM 菜单标题已可见，无需在面板中配置），删除 150 行快捷键配置相关死代码。
