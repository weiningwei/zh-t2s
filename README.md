...
[![Version](https://img.shields.io/badge/version-2.4.1-green)](CHANGELOG.md)
...
- **转换统计**：菜单与浮动按钮页眉显示当前页面已转换字符数与 OpenCC 耗时（会话级统计，方向切换时重置）。
...
- **浮动状态按钮**：右下角常驻胶囊按钮，实时显示当前状态（🟢繁→简 / 🟢简→繁 / ⚪已关闭 / ⚪已忽略 / ⚪未加载），可拖拽到任意位置并记忆，点击展开设置面板。
- **面板内一键开关**：面板顶部的开关键，关闭后一键以"关闭前的方向"重新开启（方向跨刷新记忆）。
- **设置面板**：在浮动面板中集中管理方向切换、快捷键、白名单与重置；油猴菜单仅保留方向切换、重置与显示按钮等兜底项，不再冗长。
