...
- **一键开关与方向切换**：一键开启 / 关闭或切换方向，所有标签页实时同步；切换前自动还原原文，不留残留。
- **转换统计**：菜单与浮动按钮页眉实时显示已转换字符数与耗时（切换方向时重新计数）。
- **快捷键支持**：默认 F8 开关繁→简、F9 开关简→繁，可在油猴菜单自定义；在输入框中打字时快捷键不触发，配置时还会自动避开与浏览器 / 另一方向冲突的按键。
- **白名单**：按域名排除特定站点不做转换，油猴菜单一键加入 / 移出 / 清空。
- **容错设计**：即使转换引擎的 CDN 加载失败，菜单与设置仍可正常使用，方便你管理白名单与快捷键。
- **浮动状态按钮**：右下角常驻胶囊按钮，实时显示当前状态（🟢繁→简 / 🟢简→繁 / ⚪已关闭 / ⚪已忽略 / ⚪未加载），可拖到屏幕任意位置并记住，点击展开设置面板。
